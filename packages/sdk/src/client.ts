...
export class Client {
  ...
  constructor(config: ClientConfig) {
    ...
    this.environment = config.environment ?? (process.env.VERCEL_ENV as Environment) ?? "development";
  }
  ...
  async getFlag(flagName: string, req: EvalRequest): Promise<boolean> {
    ...
    const hash = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(JSON.stringify(req)));
    ...
    return evaluate(flag, percentage, req);
  }
}
